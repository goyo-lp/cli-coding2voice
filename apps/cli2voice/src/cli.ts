...
async function installIntegration(name: 'codex' | 'claude' | 'gemini', destination?: string): Promise<void> {
  ...
  const sourceDir = path.join(repoRoot, 'integrations', name);
  const defaultDestinations: Record<typeof name, string> = {
    codex: path.join(process.env.HOME ?? repoRoot, '.codex'),
    claude: path.join(process.env.HOME ?? repoRoot, '.claude', 'plugins', 'local', 'cli2voice'),
    gemini: path.join(process.env.HOME ?? repoRoot, '.gemini', 'extensions', 'cli2voice')
  };
  const targetDir = destination ?? defaultDestinations[name];

  await copyRecursive(sourceDir, targetDir, {
    '__CLI2VOICE_ROOT__': repoRoot,
    '__CLI2VOICE_APP_CLI__': path.join(repoRoot, 'apps', 'cli2voice', 'dist', 'cli.js'),
    '__CODEX2VOICE_APP_CLI__': path.join(repoRoot, 'apps', 'codex2voice', 'dist', 'cli.js'),
    '__CLAUDE2VOICE_APP_CLI__': path.join(repoRoot, 'apps', 'claude2voice', 'dist', 'cli.js'),
    '__GEMINI2VOICE_APP_CLI__': path.join(repoRoot, 'apps', 'gemini2voice', 'dist', 'cli.js')
  });
