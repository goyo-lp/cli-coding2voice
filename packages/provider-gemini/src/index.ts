...
export async function runGeminiHookCli(argv: string[]): Promise<void> {
  const [command, ...rest] = argv;
  const client = new Cli2VoiceDaemonClient();

  if (command === 'register') {
    const sessionId = getFlag(rest, '--session-id') ?? `gemini-${process.pid}-${randomUUID()}`;
    const workspacePath = getFlag(rest, '--workspace', process.cwd()) as string;
    const session = await client.registerSession({ sessionId, provider: 'gemini', workspacePath });
  ...
  if (command === 'final') {
    ...
    await client.publishActions({ sessionId }, { actions: [{ kind: 'candidate', message, source: 'gemini.hook.final' }] });
    return;
  }
  ...
  if (command === 'signal') {
    ...
    await client.applySignal({ sessionId, provider: sessionId ? undefined : 'gemini', workspacePath, signal });
    return;
  }
